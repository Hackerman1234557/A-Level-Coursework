...
def vigenere(x):
    ...
    for letter in x:
        individualletterkey = secrets.choice(string.ascii_letters + string.punctuation + string.digits)  # this
        # generates the key for each letter as a random character in the ASCII set
...
class Button:  # This class will be used as buttons to allow the user to make selections in the program
    ...
    def isclicked(self, event):  # This method instructs the button on how to respond if it is clicked
...
class InputBox:  # This class is  to provide the user with an area to enter their text
    ...
    def handle_event(self, event):  # This method instructs the textbox on how to handle any event that may happen
        if event.type == pygame.MOUSEBUTTONDOWN:  # The box will only be active once it is clicked on
        ...
        if event.type == pygame.KEYDOWN:  # This loop allows for typed characters to be entered into the box
            if self.active:
...
def draw_window():  # this function colours the window
    ...
    encrypt_button.draw()
    decrypt_button.draw()
    pygame.display.update()  # this refreshes the window the next time the frame cycles
